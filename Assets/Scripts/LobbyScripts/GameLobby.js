...
function StartHost(password : String, players : int, serverName : String){
	...
	Network.InitializeServer((players-1), serverPort,false);	
}
...
function launchingGameGUI(){
	...
	if(Application.CanStreamedLevelBeLoaded ("GameScene")){
		...
		GUI.Label(Rect(Screen.width/4+200,Screen.height/2-25,285,150), "Starting..Loading the game: "+Mathf.Floor(Application.GetStreamProgressForLevel("GameScene")*100)+" %");
	}	
